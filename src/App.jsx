...
export default function App() {
  ...
  const handlePurchased = (amount) => {
    ...
    if (ref && (!account || ref.toLowerCase() !== account.toLowerCase())) {
      ...
      const reward = Math.round(amt * 0.1);
      const prev = Number(localStorage.getItem(key) || 0);
      localStorage.setItem(key, String(prev + reward));
      if (account && ref.toLowerCase() === account.toLowerCase()) {
        setCoinsEarned(prev + reward);
      }
    }
  };
...
// ───────────────────────────────────────────────────────────────────────────────
// THOUGHTS BOARD (stable drifting + pan + panel + share)
// ───────────────────────────────────────────────────────────────────────────────
function ThoughtsBoard({ onGetCoins, onConnect, hasHWALL, account }) {
  ...
  const [speed, setSpeed] = useState(12); // px/s
  ...
  const dragSuppressRef = useRef({ id: null, until: 0 });
  ...
  useEffect(() => {
    ...
    const step = (ts) => {
      ...
      setPosts((prev) => prev.map((p) => {
        ...
        const max = followingRef.current.has(p.author.name) ? speed * 1.2 : speed;
        const sp = Math.hypot(vx, vy);
        if (sp > max) { const k = max / (sp || 1); vx *= k; vy *= k; }
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e) => {
      ...
      const interactiveEl = closest('button, a, [role="button"], input, textarea, select, summary');
      const isInteractive = !!(interactiveEl && !(closest('.hw-card') && closest('.hw-card').isSameNode(interactiveEl)));
      const inPanel = !!(closest('.rpanel') || closest('.share-modal'));
      if (inPanel) return;
      const cardEl = closest('.hw-card');
    ...
    const onPointerUp = (e) => {
      if (cardDragRef.current && cardDragRef.current.pointerId === e.pointerId) {
        ...
        if (d.moved) { dragSuppressRef.current = { id: d.id, until: performance.now() + 300 }; }
  ...
  const toggleFollow = (name) => {
    ...
    if (selectedPost && selectedPost.author.name === name) {
      setSelectedPost((sp) => sp ? { ...sp, author: { ...sp.author, followers: Math.max(0, (sp.author.followers || 0) + (willFollow ? 1 : -1)) } } : sp);
    }
  };
  const handleReact = (postId, emo) => {
    setPosts((prev) => prev.map((p) => {
      ...
      const next = { ...p, reactions, myReact: my };
      if (selectedPost && selectedPost.id === postId) setSelectedPost(next);
      return next;
    }));
  };
  ...
  const shareLink = useMemo(() => {
    ...
    return eligible && account ? `${base}?ref=${account}` : base;
  }, [eligible, account]);
                ...
                return (
                  <div
                    ...
                    onPointerDown={(e)=>{
                      ...
                      const card = posts.find(pp=>pp.id===id);
                      if(!card) return;
                    ...
                    onPointerUp={(e)=>{
                      ...
                      if (d.moved) {
                        ...
                        const post = posts.find(pp=>pp.id===d.id);
