          ...
          return (
            <div key={p.id} className="card" onClick={()=>setSelected(p)}>
              <div style={{display:"flex",gap:10,alignItems:"center"}}>
                ...
                {followed && <span className="chip">★ Following</span>}
              </div>
              <div style={{marginTop:8, lineHeight:1.5}}>{p.text}</div>
              <div style={{marginTop:10, display:"flex", gap:8}}>
                <button className={"chip "+(p.myReact==='❤️'?"chip-on":"")} onClick={(e)=>{e.stopPropagation();react(p.id,'❤️')}}>
                  ❤️ {(p.reactions["❤️"]||0).toLocaleString()}
                </button>
                <button className="chip" onClick={(e)=>{e.stopPropagation();follow(p.author.name)}}>
                  {followingRef.current.has(p.author.name)?"✓ Following":"Follow"}
                </button>
                <button className="chip" onClick={(e)=>{e.stopPropagation();navigator.clipboard.writeText(p.text);}}>Share</button>
              </div>
            </div>
          )
        })}
          ...
          <div className="panel-author">
            ...
            <div className="a-actions">
              <button className="chip" onClick={()=>follow(selected.author.name)}>
                {followingRef.current.has(selected.author.name)?"✓ Following":"Follow"}
              </button>
            </div>
          </div>
          <div className="panel-text">{selected.text}</div>
          <div className="panel-row">
            <button className={"chip "+(selected.myReact==='❤️'?"chip-on":"")} onClick={()=>react(selected.id,'❤️')}>❤️ {(selected.reactions["❤️"]||0).toLocaleString()}</button>
